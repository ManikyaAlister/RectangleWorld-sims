...
for (i in 1:length(blocks)){
  for (j in 1:length(providers)){
    ...
    save(all_alpha_posteriors, file = here(paste0("experiment-2/modelling/04_output/b",b,"-all-alpha-posteriors-",provider,".Rdata")))
  }
}
...
all_alpha_posteriors$block <- 2
save(all_alpha_posteriors, file = here("experiment-2/modelling/04_output/tb2-all-alpha-posteriors-recursive.Rdata"))
...
all_alpha_posteriors$block <- 8
save(all_alpha_posteriors, file = here("experiment-2/modelling/04_output/tb8-all-alpha-posteriors-recursive.Rdata"))
...
for (i in 1:length(blocks)){
  for (j in 1:length(providers)){
    ...
    all_alpha_posteriors$block <- b
    save(all_alpha_posteriors, file = here(paste0("experiment-2/modelling/04_output/b",b,"-all-alpha-posteriors-",provider,"-recursive.Rdata")))
  }
}
