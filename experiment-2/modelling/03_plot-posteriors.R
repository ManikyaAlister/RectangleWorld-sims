...
for (i in 1:length(all_conditions[,1])){
  ...
  if (condition == "US" | condition == "UN"){
    load(here(paste0("experiment-",exp,"/modelling/04_output/tb8-all-alpha-posteriors-recursive.Rdata")))
  } else {
  ...
  ggsave(here(paste0("experiment-",exp,"/modelling/05_plots/posteriors-",condition,".png")), width = 7, height = 5)
}
