...
getEmpiricalPriors = function(block){
  ...
  for (i in 1:length(conds)){
    ...
    prior_scaled_i <- scaled_rectangle_sizes(prior_i, hyp) %>%
      ...
      mutate(normalised_weight = scaled_freq / sum(scaled_freq))
    
    # Add condition column
    prior_scaled_i$cond <- cond_i
    
    # Calculate density for saving
    dens <- density(prior_scaled_i$size, weights = prior_scaled_i$normalised_weight)
    sizes <- dens$x
    densities <- dens$y
    ...
    # Create plot for given condition
    dens_plot_i <- prior_scaled_i %>% 
      ggplot(aes(x = size, weight = normalised_weight)) +  # Fixed: use weight = normalised_weight
...
Not that you can't really see this well in the rendered plot, so see the original manuscript or \`plots/size-hist-priors.png\`.
...
plot_list <- plot_list[order(match(names(plot_list), conds))]
