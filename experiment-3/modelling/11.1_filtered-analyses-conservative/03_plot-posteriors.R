  ...
  for (c in clues) {
    ...
    for (i in 1:length(all_conditions_tmp[, 1])) {
      ...
      all_data <- all_alpha_posteriors %>%
        filter(cond == condition, clue == c, pid %in% good_uid_conservative) %>%
        mutate(alpha = as.factor(alpha))
      ...
      if(b %in% target_blocks){
        ggsave(here(paste0("experiment-3/modelling/11.1_filtered-analyses-conservative/plots/posteriors/posteriors-",condition,"-c-", c, "-b-",b,".png")), width = 7, height = 5, plot = plot)
      } else {
        ggsave(here(paste0("experiment-3/modelling/11.1_filtered-analyses-conservative/plots/posteriors/posteriors-",condition,"-c-", c, "-b-",b,"-",provider,".png")), width = 7, height = 5, plot = plot)
      }
    ...
    if(b %in% target_blocks){
      ggsave(here(paste0("experiment-3/modelling/11.1_filtered-analyses-conservative/plots/posteriors/posteriors-all-conditions-c-",c,"-b-",b,".png")), width = 8, height = 10, plot = combined_plot)
    } else {
      ggsave(here(paste0("experiment-3/modelling/11.1_filtered-analyses-conservative/plots/posteriors/posteriors-all-conditions-c-", c, "-b-",b,".png")), width = 8, height = 10, plot = combined_plot)
    }
