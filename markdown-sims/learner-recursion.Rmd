...
# If there is already a point distribution, this function attempts to apply a new 
  ...
  for (p in 1:nPts) {
    for (h in 1:nHyp) {
      if (isInRectangle(c(pts[p,1],pts[p,2]),c(hyp$x1[h],hyp$y1[h],hyp$x2[h],hyp$y2[h]))) {
        if (whichObs=="pos") {
          consPt[p,h] <- ptDist[p,h]*(1/hyp$size[h])^alpha
        } 
      } else {
        if (whichObs=="neg") {
...
The learener can then use that to figure out the probability of each hypothesis given the data, pLearner(h|d).
