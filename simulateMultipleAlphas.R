...
multiAlphaPredictions = function(observations, allAlphas = c(-1,0,1),H = 10, save = FALSE){
  ...
  for(alpha in 1:nAlphas) {
    ...
    if (prior == "normal") {
      lnHyps[[alpha]]$prior <- normalPrior(hyp$size, priorM, priorSD)
    }
  ...
  if (save == TRUE) {
    ...
    ggsave(plotAll, file = here(paste0("experiment-scenarios/target-blocks/figures/",scenarioCode,".png")), width = 10, height = 10)
  }
