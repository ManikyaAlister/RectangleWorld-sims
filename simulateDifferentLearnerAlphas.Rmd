---
title: "Simulate how learners with different alphas behave with the same data"
...
version <- 1
## Number of different learners in this scenario
nLearners <- 3 
# the alpha the teacher is actually following. In this example, let's make the 
# teacher helpful, but not too helpful such that they will eliminate too many 
# hypotheses too quickly. 
tchAlpha <- 0.5 
# the alpha the teacher is assuming the learner thinks the teacher is. For the 
# sake of the experiment, let's say the teacher doesn't know the learner has been
# tipped off that they may not be helpful. 
tchLnAlpha <- 0.5  
...
# Set up the different alphas the learners assume the teacher is following
learnerAlphas <- c(1,0,-1)
# as text for labeling purposes
learnerAlphasText <- paste(as.character(learnerAlphas), collapse = "_")

## Trusting learner (thinks teacher is being helpful)
lnAlphaH <- learnerAlphas[1]
## Weak learner
lnAlphaW <- learnerAlphas[2]
## Suspicious learner (thinks teacher is being deceptive)
lnAlphaD <- learnerAlphas[3]
...
## Helpful learner
lAH <- which(alphas==lnAlphaH)
lnAlphaTextH <- returnAlpha(lnAlphaH)
## Weak learner
lAW <- which(alphas==lnAlphaW)
lnAlphaTextW <- returnAlpha(lnAlphaW)
## Deceptive learner
lAD <- which(alphas==lnAlphaD)
lnAlphaTextD <- returnAlpha(lnAlphaD)
...
dirname <- paste0("x",H,"y",H,"-L",learnerAlphasText,
...
**lnAlpha**: the alpha value that the learner assumes the teacher has. Her there are three learners,  `r lnAlphaH, lnAlphaW, lnAlphaD`
...
# Initialisie points and hypotheses for the teacher
tchPts <- pts
tchHyp <- hyp

# Initialise points for different learners
lnPtsH <- pts
lnPtsW <- pts
lnPtsD <- pts

# Initialise hypotheses for different learners
lnHypH <- hyp
lnHypW <- hyp
lnHypD <- hyp
...
## Helpful learner
lnPtProbsH <- sweep(allProbPts[,,lAH],2,lnHypH$prior,"*")
lnPtsH$prior <- rowSums(lnPtProbsH)/sum(lnPtProbsH)

## Weak learner
lnPtProbsW <- sweep(allProbPts[,,lAW],2,lnHypW$prior,"*")
lnPtsW$prior <- rowSums(lnPtProbsW)/sum(lnPtProbsW)

## Deceptive learner
lnPtProbsD <- sweep(allProbPts[,,lAD],2,lnHypD$prior,"*")
lnPtsD$prior <- rowSums(lnPtProbsD)/sum(lnPtProbsD)
...
## Learner
pLnPriorsH <- plotDistribution(allPts=lnPtsH,xrange=xrange,yrange=yrange,
                         subtitle=paste0("Assumes teacher alpha=",lnAlphaH),
                         whichDist="prior",title="Priors for Trusting learner")

pLnPriorsW <- plotDistribution(allPts=lnPtsW,xrange=xrange,yrange=yrange,
                         subtitle=paste0("Assumes teacher alpha=",lnAlphaW),
                         whichDist="prior",title="Priors for Weak learner")

pLnPriorsD <- plotDistribution(allPts=lnPtsD,xrange=xrange,yrange=yrange,
                         subtitle=paste0("Assumes teacher alpha=",lnAlphaD),
                         whichDist="prior",title="Priors for Deceptive learner")

p <- ggarrange(pLnPriorsH, pLnPriorsW, pLnPriorsH,pTchPriors,ncol=2, nrow = 2)
p
...
```{r echo=FALSE, fig.height7, fig.width=12, fig.align="center", message=FALSE, warning=FALSE}

# setting subtitles
st  <- paste0("teacher thinks learner alpha: ",tchLnAlpha)
stH <- paste0("Learner alpha: ",lnAlphaH,"; teacher thinks learner alpha: ",tchLnAlpha)
stW <- paste0("Learner alpha: ",lnAlphaW,"; teacher thinks learner alpha: ",tchLnAlpha)
stD <- paste0("Learner alpha: ",lnAlphaD,"; teacher thinks learner alpha: ",tchLnAlpha)
...
# step five: learners update their estimate of the hypotheses, given the point that was generated
## Helpful learner
lnHypH <- updateHypotheses(allProbPts[,,lAH],consPts,newPt,lnHypH)
bestHH <- returnBestHypothesis(lnHypH,n=nBestH)

## Weak learner
lnHypW <- updateHypotheses(allProbPts[,,lAW],consPts,newPt,lnHypW)
bestHW <- returnBestHypothesis(lnHypW,n=nBestH)

## Deceptive learner
lnHypD <- updateHypotheses(allProbPts[,,lAD],consPts,newPt,lnHypD)
bestHD <- returnBestHypothesis(lnHypD,n=nBestH)

tH <- "Helpful Learner's actual hypothesis distribution"
tW <- "Weak Learner's actual hypothesis distribution"
tD <- "Deceptive Learner's actual hypothesis distribution"

pt1step5H <- plotHypotheses(trueH,obs,lnHypH[bestHH,],xrange=xrange,
                           yrange=yrange,title=tH,subtitle=st)

pt1step5W <- plotHypotheses(trueH,obs,lnHypW[bestHW,],xrange=xrange,
                           yrange=yrange,title=tW,subtitle=st)

pt1step5D <- plotHypotheses(trueH,obs,lnHypD[bestHD,],xrange=xrange,
                           yrange=yrange,title=tD,subtitle=st)

# step six: learner updates their estimate of the distribution over points, given the hypothesis distribution
## Strong learner
lnPtsH <- updatePoints(posProbPts[,,lAH],newPt,
                       posterior=lnHypH$posterior,pts=lnPtsH)
tH <- "Helpful Learner's actual point distribution"
pt1step6H <- plotDistribution(allPts=lnPtsH,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)

## Weak Learner
lnPtsW <- updatePoints(posProbPts[,,lAW],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsW)
tW <- "Weak Learner's actual point distribution"
pt1step6W <- plotDistribution(allPts=lnPtsW,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)

## Deceptive Learner
lnPtsD <- updatePoints(posProbPts[,,lAD],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsD)
tW <- "Deceptive Learner's actual point distribution"
pt1step6D <- plotDistribution(allPts=lnPtsD,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
pt1 <- ggarrange(pt1step1,pt1step3,pt1step4,pt1step5H,pt1step5W, pt1step5D, pt1step6H,pt1step6W, pt1step6D,ncol=3,nrow=3)
pt1
...
pt2step5H <- plotHypotheses(trueH,obs,lnHypH[bestHH,],xrange=xrange,
                           yrange=yrange,title=tH,subtitle=st)

pt2step5W <- plotHypotheses(trueH,obs,lnHypW[bestHW,],xrange=xrange,
                           yrange=yrange,title=tW,subtitle=st)

pt2step5D <- plotHypotheses(trueH,obs,lnHypD[bestHD,],xrange=xrange,
                           yrange=yrange,title=tD,subtitle=st)
...
lnPtsH <- updatePoints(posProbPts[,,lAH],newPt,
                       posterior=lnHypH$posterior,pts=lnPtsH)
tH <- "Helpful Learner's actual point distribution"
pt2step6H <- plotDistribution(allPts=lnPtsH,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
lnPtsW <- updatePoints(posProbPts[,,lAW],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsW)
tW <- "Weak Learner's actual point distribution"
pt2step6W <- plotDistribution(allPts=lnPtsW,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
lnPtsD <- updatePoints(posProbPts[,,lAD],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsD)
tW <- "Deceptive Learner's actual point distribution"
pt2step6D <- plotDistribution(allPts=lnPtsD,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
pt2 <- ggarrange(pt2step1,pt2step3,pt2step4,pt2step5H,pt2step5W, pt2step5D, pt2step6H,pt2step6W, pt2step6D,ncol=3,nrow=3)
pt2
...
pt3step5H <- plotHypotheses(trueH,obs,lnHypH[bestHH,],xrange=xrange,
                           yrange=yrange,title=tH,subtitle=st)

pt3step5W <- plotHypotheses(trueH,obs,lnHypW[bestHW,],xrange=xrange,
                           yrange=yrange,title=tW,subtitle=st)

pt3step5D <- plotHypotheses(trueH,obs,lnHypD[bestHD,],xrange=xrange,
                           yrange=yrange,title=tD,subtitle=st)
...
lnPtsH <- updatePoints(posProbPts[,,lAH],newPt,
                       posterior=lnHypH$posterior,pts=lnPtsH)
tH <- "Helpful Learner's actual point distribution"
pt3step6H <- plotDistribution(allPts=lnPtsH,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
lnPtsW <- updatePoints(posProbPts[,,lAW],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsW)
tW <- "Weak Learner's actual point distribution"
pt3step6W <- plotDistribution(allPts=lnPtsW,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
lnPtsD <- updatePoints(posProbPts[,,lAD],newPt,
                       posterior=lnHypW$posterior,pts=lnPtsD)
tW <- "Deceptive Learner's actual point distribution"
pt3step6D <- plotDistribution(allPts=lnPtsD,xrange=xrange,yrange=yrange,
                          trueRectangle=trueH,obs=obs,whichDist="posterior",
                          title=t, subtitle=st)
...
pt3 <- ggarrange(pt3step1,pt3step3,pt3step4,pt3step5H,pt3step5W, pt3step5D, pt3step6H,pt3step6W, pt3step6D,ncol=3,nrow=3)
pt3
