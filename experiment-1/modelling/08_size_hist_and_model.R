...
sizeHist = function(data){
  data %>%
    ...
    theme_classic()+
    ylim(c(0,100))+
    scale_x_discrete(unique(d_cartesian$size_resp))+
...
for (i in 1:length(all_conditions_4_8[,1])){
 ...
 sizeHist(data)
 ggsave(filename = here(paste0("experiment-1/modelling/05_plots/size_hist_b8_c4_",condition,".png")), width = 2, height = 1)
}
